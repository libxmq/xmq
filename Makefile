...
ifeq "$(DEBUG)" "true"
    ...
    DEBUG_FLAGS=-Os
    STRIP_BINARY=$(STRIP) $(BUILD)/xml2xmq
endif
...
all: $(BUILD)/xml2xmq $(BUILD)/testinternals
	@$(STRIP_BINARY)

$(BUILD)/xml2xmq: $(XMQ_OBJS) $(BUILD)/main.o
	$(CXX) -o $(BUILD)/xml2xmq $(XMQ_OBJS) $(BUILD)/main.o $(DEBUG_LDFLAGS) -lpthread
	cp $(BUILD)/xml2xmq $(BUILD)/xmq2xml
...
install:
	cp build/xml2xmq /usr/local/bin
	cp build/xmq2xml /usr/local/bin
	cp scripts/xmless /usr/local/bin
