...
then
    echo "Usage: tests/test_bad_ixml.sh [XQ_BINARY] [OUTPUT_DIR] tests/99_foo.test"
    exit 1
fi
...
$PROG --ixml=$OUTPUT/${TEST_NAME}.ixml $ARGS $OUTPUT/${TEST_NAME}.input > $OUTPUT/${TEST_NAME}.output 2>&1
