...
then
    echo "Usage: tests/test_converting.sh [XQ_BINARY] [OUTPUT_DIR] tests/99_foo.test"
    exit 1
fi
...
sed -n '/^START.*$/,/^COMPACT$/p' $TEST_FILE | tail -n +2 | sed '$d' > $OUTPUT/${TEST_NAME}.input
sed -n '/^COMPACT$/,/^XMQ$/p' $TEST_FILE | tail -n +2 | sed '$d' > $OUTPUT/${TEST_NAME}.expected_compact
